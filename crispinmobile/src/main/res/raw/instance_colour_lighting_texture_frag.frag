...
void main()
{
    ...
    for(int i = 0; i < MAX_NUM_SPOT_LIGHTS && i < uNumSpotLights; i++) {
        lightCalc += CalculateSpotLight(uSpotLights[i], normal, vFragPos, viewDirection, specularModifier, diffuseModifier);
    }
...
vec3 CalculateDirectionalLight(DirectionalLight light, vec3 normal, vec3 viewDirection, float specularModifier, vec3 diffuseModifier) {
    ...
    vec3 diffuse = uMaterial.diffuse * normalDiffuseStrength * light.colour * light.diffuse * diffuseModifier;
...
vec3 CalculateSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDirection, float specularModifier, vec3 diffuseModifier) {
    ...
    // Diffuse lighting calculation
    float normalDiffuseStrength = max(dot(normal, lightDirection), 0.0);
    vec3 diffuse = uMaterial.diffuse * normalDiffuseStrength * light.colour * light.diffuse * attenuation * spotlight * diffuseModifier;
