...
public class HitboxPolygon {
    ...
    private void calculateAxes() {
        float[] points = transformedPoints == null ? data : transformedPoints;
        ...
        for (int i = 0; i < points.length; i += 2) {
            ...
            float length = (float) Math.sqrt(Math.abs((edgeY
                    * edgeY) + (edgeX * edgeX)));
            axes[i] = edgeY / length;
    ...
    public boolean isCollidingSAT(HitboxPolygon other) {
        float[] points = transformedPoints == null ? data : transformedPoints;
        float[] otherPoints = other.transformedPoints == null ? other.data : other.transformedPoints;
        ...
        for (int i = 0; i < axes.length; i += 2) {
            projectPolygon(proj1, axes[i], axes[i + 1], points);
            projectPolygon(proj2, axes[i], axes[i + 1], otherPoints);
        ...
        for (int i = 0; i < other.axes.length; i += 2) {
            projectPolygon(proj1, other.axes[i], other.axes[i + 1], points);
            projectPolygon(proj2, other.axes[i], other.axes[i + 1], otherPoints);
            if (!overlap(proj1, proj2)) {
                return false;
            }
        }
