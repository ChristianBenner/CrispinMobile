...
public class LayeredSpatialHashGrid2DBase<T extends InlaidClient> {
    private LayeredCellBase[][] cells;
    ...
    public LayeredSpatialHashGrid2DBase(Vec2 start, Scale2D size, int cellsPerRow, int cellsPerColumn, int numLayers) {
        ...
        cells = new LayeredCellBase[cellsPerColumn][cellsPerRow];
        for (int x = 0; x < cellsPerRow; x++) {
            for (int y = 0; y < cellsPerColumn; y++) {
                cells[y][x] = new LayeredCellBase();
            }
        }
    }

    public HashSet<T>[] findNear(float x, float y, float w, float h) {
        // Get the cell co-ordinates for the bounds of the query
        int[] i1 = getCellIndex(x, y);
        int[] i2 = getCellIndex(x + w, y + h);

        HashSet<T>[] clientLayers = new HashSet[numLayers];
        ...
        for (int ix = i1[0], xn = i2[0]; ix <= xn; ix++) {
            for (int iy = i1[1], yn = i2[1]; iy <= yn; iy++) {
                LayeredCellBase<T> cell = cells[iy][ix];
                for(T client : cell) {
    ...
    public HashSet<T>[] findNear(Vec2 position, Scale2D size) {
        return findNear(position.x, position.y, size.w, size.h);
    }

    public HashSet<T>[] findNear(Camera2D camera2D) {
